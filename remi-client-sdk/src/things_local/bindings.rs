...
impl<'a> ThingsLocalService<'a> {
    pub fn set_collection_trigger_uuid(
        &self,
        device_id: &str,
        collection_uuid: &str,
        trigger_uuid: Option<&str>,
    ) -> Result<ThingsMutationResult<()>> {
        self.patch_collection_trigger_uuid(
            device_id,
            collection_uuid,
    ...
    pub fn patch_collection_trigger_uuid(
        &self,
        device_id: &str,
        collection_uuid: &str,
        ...
        let mut doc_set = DocumentPersistence::new(self.storage)
            ...
            .context("Failed to load Things document set before collection trigger update")?;

        let snapshot = doc_set.extract_snapshot()?;
        if snapshot
            .collections
            .iter()
            .all(|collection| collection.uuid != collection_uuid)
        {
            anyhow::bail!("Collection not found: {collection_uuid}");
        }

        let trigger = crate::things_crdt::trigger_update_from_field_patch(trigger_uuid);
    ...
    pub fn set_thing_trigger_uuid(
        &self,
        device_id: &str,
        thing_uuid: &str,
        trigger_uuid: Option<&str>,
    ) -> Result<ThingsMutationResult<()>> {
        self.patch_thing_trigger_uuid(
            device_id,
            thing_uuid,
            FieldPatch::from_compat_option_str(trigger_uuid),
        )
    ...
    pub fn patch_thing_trigger_uuid(
        &self,
        device_id: &str,
        thing_uuid: &str,
        trigger_uuid: FieldPatch<String>,
        ...
        let mut doc_set = DocumentPersistence::new(self.storage)
            .load_or_init_document_set(device_id)
            .context("Failed to load Things document set before thing trigger update")?;
